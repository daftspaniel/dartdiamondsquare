...
class World{
  ...
  void SetCorners(int x, int y, int w, int h,  [List v = null]){
    ...
    int hw = (w/2).floor();
    int hh = (h/2).floor();    
    int total = map_data[x][y] + map_data[x+w][y] + map_data[x][y+h] + map_data[x+w][y+h] + rng.nextInt(4+h) + 4;
  ...
  void Smooth(){
    ...
    for (int yl = 1; yl<(Length-1);yl++){
      for (int xl = 1; xl<(Width-1);xl++){
        average = (1.1*( (map_data[xl][yl] + map_data[xl+1][yl] + map_data[xl][yl+1] + map_data[xl+1][yl+1])/4)).floor();
